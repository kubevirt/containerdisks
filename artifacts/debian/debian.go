...
func decodeSHA(base64Checksum string) (string, error) {
...
func (d *debian) getBuildData(jsonURL string) (additionalTags []string, checksum string, err error) {
	...
	if err != nil {
		return []string{}, "", fmt.Errorf("error downloading debian json file: %v", err)
	}
	err = json.Unmarshal(raw, &buildData)
	if err != nil {
		return []string{}, "", fmt.Errorf("error decoding debian json file: %v", err)
	}

	if len(buildData.Items) == 0 {
		return []string{}, "", fmt.Errorf("build debian data not found")
	}

	// Usually the qcow is on the 3rd item. However, let's be sure that it is the qcow image digest.
	for _, item := range buildData.Items {
		if item.Metadata.Labels.ImageFormat != "" && item.Metadata.Labels.ImageFormat == "qcow2" {
			additionalTags = append(additionalTags, d.Version+"-"+item.Metadata.Labels.Version)
			checksum, err = decodeSHA(item.Metadata.Annotations.Digest)
			return additionalTags, checksum, err
		}
	}

	return additionalTags, checksum, fmt.Errorf("error locating the image information")
}
...
func (d *debian) Inspect() (*api.ArtifactDetails, error) {
	...
	if strings.HasPrefix(d.Version, "11") || strings.HasSuffix(d.Version, "12") {
		baseURL = fmt.Sprintf(baseURLFmt, d.VersionName)
		baseImageName = fmt.Sprintf(baseNameFmt, d.Version, architecture.GetImageArchitecture(d.Arch))
	} else if strings.HasPrefix(d.Version, "13") {
		// Debian 13 is still on "testing" phase. These builds are uploaded on daily basis to a different folder.
		// This code should be adjusted once it reaches "stable" phase.
		baseURL = fmt.Sprintf(baseURLFmt, d.VersionName+"/daily")
		baseImageName = fmt.Sprintf(baseNameFmt, d.Version, architecture.GetImageArchitecture(d.Arch)+"-daily")
	} else {
		return nil, fmt.Errorf("can't understand provided version %s", d.Version)
	}

	additionalTags, checksum, err := d.getBuildData(baseURL + baseImageName + ".json")
	if err != nil {
		return nil, err
	}
	...
	return &api.ArtifactDetails{
		SHASum:               checksum,
		DownloadURL:          baseURL + baseImageName + ".qcow2",
